...
/**
 * Name: AcademyCraft2 / OnlyIfAdapter
 * Author: cnlimiter
 * CreateTime: 2023/12/23 21:18
 * Description:
 */
...
public class OnlyIfAdapter {
    private static boolean recursiveGetBool(Object inst, Object objInQuestion, ResourceLocation objId, Class<?> c, String... subs)
    {
        try
        {
            ...
            {
                Method m = ReflectionUtils.findDeclaredMethod(c, subs[0], mt -> mt.getParameterCount() == 0 || mt.getParameterCount() == 1 || mt.getParameterCount() == 2);
        ...
        if(onlyIf != null)
        {
            ...
            } catch(RuntimeException e)
            {
                ...
                while(err.getCause() != null)
                    err = err.getCause();
                err.printStackTrace();
                Debug.warn("Failed to parse @OnlyIf({}) in {}! {} will" + (onlyIf.invert() ? " not be registered due to inversion." : " be registered anyway."), JSONObject.quote(member), source, type);
            }
    ...
    {
        Supplier<Class<?>> owner = Suppliers.memoize(() -> ReflectionUtils.fetchClass((Type) onlyIf.get("owner")));
