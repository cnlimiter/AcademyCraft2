...
import cn.evole.mods.academy.utils.java.Fetcher;
import cn.evole.mods.academy.utils.java.functions.Function3;
...
/**
 * Name: AcademyCraft2 / ScanDataUtil
 * Author: cnlimiter
 * CreateTime: 2023/12/23 22:38
 * Description:
 */
...
public class ScanDataUtils {
    ...
    public static <T> List<Class<? extends T>> lookupAnnotatedTypes(Class<? extends Annotation> annotation, Class<T> baseType, Predicate<ModAwareAnnotationData> matcher)
    {
        ...
        lookupAnnotatedObjects(annotation,
                ad -> ad.parent.targetType() == ElementType.TYPE &&
                        baseType.isAssignableFrom(ReflectionUtils.fetchClass(ad.parent.clazz())) && matcher.test(ad)
        ) //
                .forEach(ad -> classes.add(ReflectionUtils.fetchClass(ad.parent.clazz())));
        return classes;
    }
    ...
    public static class ModAwareAnnotationData
    {
        ...
        {
            return ReflectionUtils.fetchClass(parent.clazz());
        }
