...
public class RenderUtils {
    ...
    public static void renderCenter(GuiGraphics graphics, int drawWidth, int drawHeight, int width, int height, ResourceLocation resource) {
        renderCenter(graphics, 0, 0, drawWidth, drawHeight, width, height, resource);
    }

    public static void renderCenter(GuiGraphics graphics, int x, int y, int drawWidth, int drawHeight, int width, int height, ResourceLocation resource) {
        int left = (width - drawWidth) / 2;
        int top = (height - drawHeight) / 2;
        render(graphics, drawWidth, drawHeight, left + x, top + y, resource);
    }

    public static void renderCenter(GuiGraphics graphics, int x, int y, int drawWidth, int drawHeight,int drawTextureWidth, int drawTextureHeight, int width, int height, ResourceLocation resource, int textureStartX, int textureStartY, int textureWidth, int textureHeight) {
        int left = (width - drawWidth) / 2;
        int top = (height - drawHeight) / 2;
        render(graphics, drawWidth, drawHeight,drawTextureWidth,drawTextureHeight, left + x, top + y, resource, textureStartX, textureStartY, textureWidth, textureHeight);
    }

    public static void renderCenterTop(GuiGraphics graphics, int x, int y, int drawWidth, int drawHeight, int width, int top, ResourceLocation resource) {
        int left = (width - drawWidth) / 2;
        render(graphics, drawWidth, drawHeight, left + x, top + y, resource);
    }


    public static void render(GuiGraphics graphics, int drawWidth, int drawHeight, int left, int top, ResourceLocation resource) {
        //drawWidth和drawHeight名字反了，值没反
        graphics.blit(resource, left, top, 0, 0, 0, drawWidth, drawHeight, drawWidth, drawHeight);
    }

    public static void render(GuiGraphics graphics, int drawWidth, int drawHeight,int drawTextureWidth, int drawTextureHeight, int left, int top, ResourceLocation resource, int textureStartX, int textureStartY, int textureWidth, int textureHeight) {
        graphics.blit(resource, left, top, drawWidth, drawHeight, textureStartY, textureStartX, drawTextureWidth, drawTextureHeight, textureWidth, textureHeight);
    }

    public static void renderText(GuiGraphics graphics, String text, int x, int y) {
        renderText(graphics, text, x, y, Style.EMPTY);
    }

    public static void renderText(GuiGraphics graphics, String text, int x, int y, Style style) {
